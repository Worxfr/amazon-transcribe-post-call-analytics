...
def lambda_handler(event, context):
    ...
    if SUMMARIZE_TYPE == 'SAGEMAKER':
        ...
        try:
            print(f"BEDROCK summarization starting with standard prompt templates")
            print(f"Transcript preview (first 200 chars): {transcript_str[:200]}...")
            
            # Use standard prompt templates for all content (including Dutch)
            # This will use the updated DynamoDB prompt templates with English field names and Dutch content
            # Note: Dutch NLP is still used for sentiment analysis in turn-by-turn processing
            print("Using standard prompt templates for summarization (Dutch NLP preserved for sentiment analysis)")
            summary = generate_bedrock_summary(transcript_str, pca_results.analytics.transcribe_job.api_mode)
            try: 
                summary_json = json.loads(summary)
                print(f"Standard summary generated successfully with English field names")
            except:
                print('no json detected in summary.')
        except Exception as err:
